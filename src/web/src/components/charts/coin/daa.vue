  ...
  export default {
    ...
    methods: {
      ...
      redraw () {
        ...
        let ts = 1510602000
        let d = this.data.BCH.history['all']
        ...
        this.ts = ts
        this.blocks = this.data.BCH.history['all'].reduce((blocks, obj) => {
          if (obj.timestamp > ts) {
            blocks += obj.blocks
          }
          return blocks
        ...
        this.dari_low = this.data.BCH.history['all'].reduce((blocks, obj) => {
          if (obj.timestamp > ts) {

            let btc = this.ts_coin("BTC", obj.timestamp)
        ...
        this.dari_high = this.data.BCH.history['all'].reduce((blocks, obj) => {
          if (obj.timestamp > ts) {
        ...
        this.blocks_min = this.data.BCH.history['all'].reduce((blocks, obj) => {
          if (obj.timestamp > ts) {
        ...
        this.blocks_max = this.data.BCH.history['all'].reduce((blocks, obj) => {
          if (obj.timestamp > ts) {
            if (obj.blocks > blocks) {
              blocks = obj.blocks
            }
        ...
        this.data.BCH.history['all'].forEach((obj) => {
          if (obj.timestamp > ts) {
            if (obj.blocks === 0) {
              this.times['x'] += 1
            }
        ...
        let labels = this.data.BCH.history['all'].reduce((dates, obj) => {
          if (obj.timestamp > ts) {
        ...
        let dari = (vals, obj) => {
          if (obj.timestamp > ts) {
            vals.push(obj.blocks > 0 ? this.sprintf('%.2f', obj.dari / this.ts_coin('BTC', obj.timestamp).dari) : null)
          }
          return vals
        }
        ...
        let blks_hi = (vals, obj) => {
          if (obj.timestamp > ts) {
            let rat =  obj.dari / this.ts_coin('BTC', obj.timestamp).dari
        ...
        this.add_dataset({
          ...
          data: this.data.BCH.history['all'].reduce(act, []),
          spanGaps: true,
          yAxisID: 'diff',
          borderDash: [3, 3],
        }, (d) => {
        ...
        /*        this.add_dataset({
                  ...
                  label: 'Perfect',
                  borderColor: '#ff0000',
                  data: this.data.BCH.history['all'].reduce(perf, []),
                  spanGaps: true,
                  yAxisID: 'diff'
                }, (d) => {
                  d.lineTension = 0.5
        ...
        this.add_dataset({
          type: 'line',
          label: 'DARI ratio vs BTC',
          borderColor: '#ff0000',
          fill: false,
          borderWidth: 1,
          data: this.data.BCH.history['all'].reduce(dari, []),
          yAxisID: 'dari'
        }, (d) => {
          d.lineTension = 0.2
        })
        ...
        this.add_dataset({
          ...
          fill: true,
          borderWidth: 0,
          data: this.data.BCH.history['all'].reduce(blks, []),
          yAxisID: 'ratio'
        })
...
/*        this.add_dataset({
          ...
          fill: true,
          borderWidth: 0,
          data: this.data.BCH.history['all'].reduce(blks_lo, []),
          yAxisID: 'ratio'
        })
        ...
        this.add_dataset({
          ...
          fill: true,
          borderWidth: 0,
          data: this.data.BCH.history['all'].reduce(blks_hi, []),
          yAxisID: 'ratio'
        })*/
