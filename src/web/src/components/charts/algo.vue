<template>
  <div>
    <div style="height:512px; width: 100%;">
      <canvas :id="'canvas-algo-' + coin" style="height: 512px;"></canvas>
    </div>
    <br>
    <p>
    ...
    <p>
      ...
      <small>
        Number of blocks mined during periods of relative block reward per amount of work done compared to {{ coino(coin)
        }}.
        <table class="table table-sm table-bordered times-table">
          <thead class="thead-dark chead">
          <tr>
  ...
  export default {
    ...
    methods: {
      ...
      redraw () {
        ...
        let ts = 0
        let d = this.data[this.coin].history['all']
        ...
        this.ts = ts
        this.blocks = this.data[this.coin].history['all'].reduce((blocks, obj) => {
          if (obj.timestamp > ts) {
            blocks += obj.blocks
          }
          return blocks
        ...
        this.dari_low = this.data[this.coin].history['all'].reduce((blocks, obj) => {
          if (obj.timestamp > ts) {

            let btc = this.ts_coin(this.coino(this.coin), obj.timestamp)
        ...
        this.dari_high = this.data[this.coin].history['all'].reduce((blocks, obj) => {
          if (obj.timestamp > ts) {
        ...
        this.blocks_min = this.data[this.coin].history['all'].reduce((blocks, obj) => {
          if (obj.timestamp > ts) {
        ...
        this.blocks_max = this.data[this.coin].history['all'].reduce((blocks, obj) => {
          if (obj.timestamp > ts) {
            if (obj.blocks > blocks) {
              blocks = obj.blocks
            }
        ...
        this.data[this.coin].history['all'].forEach((obj) => {
          if (obj.timestamp > ts) {
            if (obj.blocks === 0) {
              this.times['x'] += 1
            }
        ...
        let labels = this.data[this.coin].history['all'].reduce((dates, obj) => {
          if (obj.timestamp > ts) {
        ...
        let perf = (vals, obj) => {
          if (obj.timestamp > ts) {
            ...
            let o = this.tsrate(this.coino(this.coin), obj.timestamp)
            if (!o) o = 0
        ...
        let dari = (vals, obj) => {
          if (obj.timestamp > ts) {
            vals.push(obj.blocks > 0 ? this.sprintf('%.2f', obj.dari / this.ts_coin(this.coino(this.coin), obj.timestamp).dari) : null)
          }
          return vals
        }
        ...
        let blks_hi = (vals, obj) => {
          if (obj.timestamp > ts) {
            let rat = obj.dari / this.ts_coin(this.coino(this.coin), obj.timestamp).dari
        ...
        this.add_dataset({
          ...
          data: this.data[this.coin].history['all'].reduce(act, []),
          spanGaps: true,
          yAxisID: 'diff',
          borderDash: [3, 3],
        }, (d) => {
        ...
        this.add_dataset({
          type: 'line',
          label: 'DARI ratio vs ' + this.coino(this.coin),
          borderColor: '#ff0000',
          fill: false,
          borderWidth: 1,
          data: this.data[this.coin].history['all'].reduce(dari, []),
          yAxisID: 'dari'
        }, (d) => {
          d.lineTension = 0.2
        })
        ...
        this.add_dataset({
          ...
          fill: true,
          borderWidth: 0,
          data: this.data[this.coin].history['all'].reduce(blks, []),
          yAxisID: 'ratio'
        })
        ...
        this.draw('canvas-algo-' + this.coin, config, labels)
      }
    }
  }
</script>
